...
// need to have this if compiler is set to unicode
const char* conv2mb(LPCTSTR str) {
...
// ***************************************************************************
bool CWinSock2Async::Create( int nPort )
{
	...
	//
	u_long nRemoteAddress = LookupAddress(L"127.0.0.1");
...
u_long CWinSock2Async::LookupAddress( LPCTSTR szHost )
{
    ...
    u_long nRemoteAddr = inet_addr( conv2mb(szHost) );
    if( nRemoteAddr == INADDR_NONE ) 
	{
        // pcHost isn't a dotted IP, so resolve it through DNS
		// WARNING : This takes time and so will stop processing!
        hostent* pHE = gethostbyname( conv2mb(szHost) );
        if (pHE == 0) 
		{
            return INADDR_NONE;
        }
